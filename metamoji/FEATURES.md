...
- [x] 先生からのメッセージ受信
- [x] 所属クラスボックスの一覧(`/drives/entry`)
- [x] クラスボックスの中身の一覧(`SdCloudService`、ドライブ別セッション)
- [x] クラスボックス内のフォルダ(`absPath` の階層 + `childrenOrder` の所属)
- [x] クラスボックスのノートを開く(端末に複製、`.atdoc` インポータを再利用)
- [x] サインインの永続化(Cookie ジャー + 身元 + 資格情報、`0600`)
- [ ] クラスボックスへの書き戻し — 編集の送信と同じくライターが必要

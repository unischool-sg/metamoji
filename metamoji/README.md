...
**セッションは永続化する。** アプリデータディレクトリの `session.json`(Unix では
`0600`)に Cookie ジャー・身元・資格情報を書く。起動のたびにサインインを求めるのは
セキュリティ姿勢ではなく単なる障害物で、元アプリも `CsDCUserInfoSettings` に
資格情報を保存して `executeWithAutoLoginFor` で黙って再認証している。

資格情報まで保存しているのは `cosmos/*`(教室のルーム)が Cookie ではなく
**リクエストごとに** JSON パートで認証するため。Cookie だけ保存すると、
再起動後に教室だけ壊れて他は動くという分かりにくい壊れ方になる。

⚠️ `cookie_store` の `save` は**非永続 Cookie を黙って捨てる**。ログインセッション
Cookie には `Expires` が無いので、まさにそれが捨てられる。
`save_incl_expired_and_nonpersistent` を使うこと ── ここを間違えると
「保存しているのにセッションだけ消える」という症状になる。
...
サーバーは自分の都合でセッションを切る。切れた後の呼び出しは
`errorCode 0x6a` と `"It doesn't log it in."` を返すが、これをそのまま画面に
出すのは間違い ── ユーザーは自分の名前がタイトルバーに出ている状態でそれを
読むことになる。

`AppError::NotLoggedIn` という専用のバリアントにして、呼び出し側が保存済みの
資格情報で**黙って再ログインし、1回だけ再試行**する。元アプリの
`executeWithAutoLoginFor` と同じ振る舞いで、資格情報を保存している理由でもある。
ドライブ側(`0x2af9`)も同様。再ログイン自体が失敗したら、そこで初めて
ユーザーに出す。

再ログインには**どちらの方式でサインインしたか**が要る(簡易ログインの
アカウントには `/users3/login` に送る `loginName` が無い)。記録前のビルドで
作られたセッションのために、`loginName` の有無から方式を復元する経路も残して
ある ── アップデートが「もう一度サインインしてください」に見えては困る。

### 所属クラスの一覧

`GET /drives/entry` が、そのアカウントが所属するドライブ(=クラスボックス)を返す。
これが無いと入口が参加コードだけになり、生徒は毎朝コードをもらう羽目になる
── 所属はサーバー側にあるのだから、聞き直すのはアプリが忘れているだけ。

エントリのキーは `DvmDriveBean` が読むもの: `id`(`driveId` ではない)/ `name` /
`groupId` / `teamId` / `hidden`。`hidden` は**整数の 1 のみ**が非表示で、
それ以外(未設定・0・文字列の "1")は表示。取り違えると一覧が空になる。

`/drives/entry` と `/drives/{id}/home` は **GET** で、しかも JSON ボディを送る
(`docs/typespec/README.md` §通信の基本仕様)。POST で投げると別物として扱われる。

### クラスボックスの中身

クラスボックスは**ドライブ**で、その中身は `SdCloudService` という別サービスの
管轄。参加コードで得られるのは所属で、ノートを得るのはこちら。

```
GET  /drives/{driveId}/home            → homeDir(そのドライブ専用のベース URL)
POST {homeDir}rest/users/login         → ドライブ側の別セッション
GET  {homeDir}rest/drives/{id}/data    → 中身(zip)
GET  {homeDir}rest/drives/{id}/documents/{docId}/data → ノート本体
```
...
**フォルダは ID ではなくパス、所属はタグ。** `SdMOFolder` は `absPath` で
識別されるので、階層は文字列の中にあり結合が要らない。**ノートは自分の
`tags` フィールドで所属フォルダを名乗る** ── 同じ `/` 区切りのパスで、
`SdDriveSyncProcess$19$1` はこれを文字列として読んで
`SdUtils.tagsFromPath` に渡している。関数名がそのまま「パスはタグ」だと
言っていた。

`childrenOrder` は**所属ではなく並び順**で、しかもフォルダのレコードでは
なく `childrenorders_` という別エントリにある(`$18$1`)。ここから所属を
読むと、階層は正しいのに**ノートが全部最上位に出る** ── 下り方向では
この項目が空だから。

`parentAbsPath` はフォルダレコードにそのまま入っているので、パスから
推測せずそれを使う(サーバーと違う推測をする余地が無くなる)。
タグの無いノートは最上位に置く ── 隠すよりましなので。

「このアプリのものは何でもモデルコンテナだろう」と決めつけて `.atdoc`
インポータに通していたが、症状は
`archive contains no MetaMoJi document` ── ダウンロードが壊れたように
読める形で出る。ノート本体のほうは実際にモデルコンテナなので、そちらは
インポータで正しい。

`.json` でないエントリは**名前を画面に出す**。読めないクラスボックスと
空のクラスボックスは、先生にとってまったく別物なので。
